    ...
    {tab === 'pickup' ? <section className="pickup-scanner a4-page-section"><div className="pickup-scanner__icon"><QrCodeScannerRounded/></div><h2>سكان رمز الحجز</h2><p>امسح الرمز الموجود على ريسيت الحجز أو الصقه في الحقل التالي.</p><div className="pickup-scanner__form"><TextField label="رمز استلام الحجز" autoFocus value={scan} onChange={e => setScan(e.target.value)} onKeyDown={e => e.key === 'Enter' && scanPreorder()} placeholder="pre_xxxxxxxxxxxxxxxxx" slotProps={{ htmlInput: { dir: 'ltr' } }} fullWidth/><Button variant="contained" onClick={() => scanPreorder()} disabled={loading}>عرض الحجز</Button></div></section> : <>
      ...
      <div className="pos-workspace">
        ...
        <aside className="pos-cart a4-page-section"><div className="a4-toolbar"><div><h2 className="a4-section-title">{tab === 'preorder' ? 'منتجات الحجز' : 'سلة البيع'}</h2><p className="a4-section-subtitle">{number(cart.reduce((s,i) => s + i.quantity, 0))} قطعة</p></div><ShoppingCartRounded color="primary"/></div>
          ...
          {tab === 'preorder' && <div className="pos-customer"><Field label="اسم العميل" required><TextField size="small" value={customer.customerName} onChange={e => setCustomer(v => ({ ...v, customerName: e.target.value }))}/></Field><Field label="رقم الهاتف" required><TextField size="small" slotProps={{ htmlInput: { dir: 'ltr' } }} value={customer.customerPhone} onChange={e => setCustomer(v => ({ ...v, customerPhone: e.target.value }))}/></Field><Field label="طريقة الاستلام"><TextField select size="small" value={customer.pickupMethod} onChange={e => setCustomer(v => ({ ...v, pickupMethod: e.target.value }))}><MenuItem value="walk_in">استلام من المكتبة</MenuItem><MenuItem value="delivery">توصيل</MenuItem></TextField></Field></div>}
          <div className="pos-summary"><div><span>المجموع الفرعي</span><strong>{money(subtotal)}</strong></div><div><span>الخصم</span><TextField label="الخصم بالجنيه" size="small" type="number" value={discountEgp} onChange={e => setDiscountEgp(e.target.value)} slotProps={{ htmlInput: { min: 0, step: .01 } }} sx={{ width: 120 }}/></div>{tab === 'preorder' && <div><span>الحد الأدنى للعربون</span><strong>{money(depositRequired)}</strong></div>}<div className="pos-summary__total"><span>{tab === 'preorder' ? 'المطلوب الآن' : 'الإجمالي'}</span><strong>{money(due)}</strong></div></div>
