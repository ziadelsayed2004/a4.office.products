...
export default function POS() {
  ...
  return <div className="a4-page pos-page">
    ...
    {tab === 'pickup' ? <section className="pickup-scanner a4-page-section"><div className="pickup-scanner__icon"><QrCodeScannerRounded/></div><h2>سكان رمز الحجز</h2><p>امسح الرمز الموجود على ريسيت الحجز أو الصقه في الحقل التالي.</p><div className="pickup-scanner__form"><Field className="pickup-scanner__field" label="رمز استلام الحجز" ltr><TextField autoFocus value={scan} onChange={e => setScan(e.target.value)} onKeyDown={e => e.key === 'Enter' && scanPreorder()} placeholder="pre_xxxxxxxxxxxxxxxxx"/></Field><Button variant="contained" onClick={() => scanPreorder()} disabled={loading}>عرض الحجز</Button></div></section> : <>
      <section className="pos-scanbar"><Field label="رمز المنتج أو الباركود" ltr><TextField inputRef={scanRef} value={scan} onChange={e => setScan(e.target.value)} onKeyDown={e => e.key === 'Enter' && scanCode()} placeholder="امسح رمز المنتج أو اكتب SKU / الباركود" slotProps={{ input: { startAdornment: <InputAdornment position="start"><QrCodeScannerRounded color="primary"/></InputAdornment>, endAdornment: <InputAdornment position="end"><Button size="small" onClick={scanCode}>إضافة</Button></InputAdornment> } }}/></Field></section>
      <div className="pos-workspace">
        <section className="pos-catalog a4-page-section"><div className="a4-toolbar"><div><h2 className="a4-section-title">البحث عن المنتجات</h2><p className="a4-section-subtitle">النتائج النشطة المتاحة للبيع أو الحجز.</p></div></div><div className="pos-search"><Field className="pos-search__field" label="البحث عن منتج"><TextField value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && searchProducts()} placeholder="ابحث باسم المنتج أو الكود" slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchRounded/></InputAdornment> } }}/></Field><Button variant="outlined" onClick={() => searchProducts()} disabled={searching}>بحث</Button></div>
        ...
        <aside className="pos-cart a4-page-section"><div className="a4-toolbar"><div><h2 className="a4-section-title">{tab === 'preorder' ? 'منتجات الحجز' : 'سلة البيع'}</h2><p className="a4-section-subtitle">{number(cart.reduce((s,i) => s + i.quantity, 0))} قطعة</p></div><ShoppingCartRounded color="primary"/></div>
          <div className="pos-cart__items">{cart.length ? cart.map((item,index) => <article className="pos-cart-item" key={`${item.product.id}-${index}`}><div className="pos-cart-item__head"><div><strong>{item.product.name}</strong><span className="a4-ltr">{item.product.sku}</span></div><IconButton size="small" color="error" onClick={() => setCart(v => v.filter((_,i) => i !== index))}><DeleteOutlineRounded fontSize="small"/></IconButton></div><Field label="فئة السعر" density="compact"><TextField select value={item.priceTierId} onChange={e => updateTier(index,e.target.value)}>{item.product.prices.map(p => <MenuItem key={p.price_tier_id} value={p.price_tier_id}>{p.tier_name} — {money(p.price)}</MenuItem>)}</TextField></Field><div className="pos-cart-item__bottom"><div className="qty-control"><IconButton size="small" onClick={() => updateQty(index,-1)}><RemoveRounded/></IconButton><strong>{number(item.quantity)}</strong><IconButton size="small" onClick={() => updateQty(index,1)}><AddRounded/></IconButton></div><b>{money(item.quantity * item.unitPrice)}</b></div></article>) : <EmptyState title="السلة فارغة" description="أضف المنتجات بالسكان أو من نتائج البحث."/>}</div>
          {tab === 'preorder' && <div className="pos-customer"><Field label="اسم العميل" required density="compact"><TextField value={customer.customerName} onChange={e => setCustomer(v => ({ ...v, customerName: e.target.value }))}/></Field><Field label="رقم الهاتف" required density="compact" ltr><TextField value={customer.customerPhone} onChange={e => setCustomer(v => ({ ...v, customerPhone: e.target.value }))}/></Field><Field label="طريقة الاستلام" density="compact"><TextField select value={customer.pickupMethod} onChange={e => setCustomer(v => ({ ...v, pickupMethod: e.target.value }))}><MenuItem value="walk_in">استلام من المكتبة</MenuItem><MenuItem value="delivery">توصيل</MenuItem></TextField></Field></div>}
          <div className="pos-summary"><div><span>المجموع الفرعي</span><strong>{money(subtotal)}</strong></div><div><span>الخصم</span><Field className="pos-summary__discount-field" label="الخصم بالجنيه" density="compact"><TextField type="number" value={discountEgp} onChange={e => setDiscountEgp(e.target.value)} slotProps={{ htmlInput: { min: 0, step: .01 } }}/></Field></div>{tab === 'preorder' && <div><span>الحد الأدنى للعربون</span><strong>{money(depositRequired)}</strong></div>}<div className="pos-summary__total"><span>{tab === 'preorder' ? 'المطلوب الآن' : 'الإجمالي'}</span><strong>{money(due)}</strong></div></div>
