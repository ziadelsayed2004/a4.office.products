...
function Protected({ children }) { const { loading, isAuthenticated } = useAuth(); if (loading) return <LoadingState label="جاري تجهيز المنصة..."/>; return isAuthenticated ? children : <Navigate to="/login" replace/>; }
function AdminOnly({ children }) { const { loading, isAuthenticated, isAdmin } = useAuth(); if (loading) return <LoadingState/>; if (!isAuthenticated) return <Navigate to="/login" replace/>; return isAdmin ? children : <Navigate to="/pos" replace/>; }
function GuestOnly({ children }) { const { loading, isAuthenticated, isAdmin } = useAuth(); if (loading) return <LoadingState/>; return isAuthenticated ? <Navigate to={isAdmin ? '/' : '/pos'} replace/> : children; }
function Home() { const { isAdmin } = useAuth(); return isAdmin ? <Dashboard/> : <Navigate to="/pos" replace/>; }

export default function App() {
  return <AppTheme><AuthProvider><BrowserRouter><Suspense fallback={<LoadingState label="جاري فتح الصفحة..."/>}><Routes>
  ...
  </Routes></Suspense></BrowserRouter></AuthProvider></AppTheme>;
}
