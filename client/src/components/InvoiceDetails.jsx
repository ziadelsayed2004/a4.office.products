...
export function InvoiceDetails({ invoice, isAdmin = false }) {
  ...
  const customerName = first(invoice.customer_name_snapshot, invoice.customer_name, invoice.customer?.name);
  const customerPhone = first(invoice.customer_phone_snapshot, invoice.customer_phone, invoice.customer?.phone);
    ...
    <div className="invoice-details">
      <div className="invoice-details__metrics">
        <Paper variant="outlined" className="invoice-detail-card">
          ...
          <strong className="a4-ltr">{first(invoice.invoice_number, invoice.invoiceNumber, `#${invoice.id}`)}</strong>
      ...
      <section className="invoice-detail-section">
        ...
        <div className="invoice-detail-grid">
          <DetailRow label="الكاشير" value={first(invoice.cashier_name_snapshot, invoice.cashier_name, invoice.cashier?.name)} />
          <DetailRow label="الشيفت" value={first(invoice.shift_id, invoice.shift?.id) ? `#${first(invoice.shift_id, invoice.shift?.id)}` : '—'} />
          ...
          <DetailRow label="رقم الحجز المرتبط" value={first(preorder?.preorder_number, invoice.preorder_number, '—')} ltr />
        </div>
      </section>
      ...
      <section className="invoice-detail-section">
        ...
        <div className="invoice-detail-list">
          {items.length ? items.map((item, index) => {
            ...
            const unitPrice = Number(first(item.unit_price_snapshot, item.unit_price, item.price, 0));
            return (
              <article className="invoice-detail-list__item" key={first(item.id, `${item.product_id}-${index}`)}>
                <div>
                  <strong>{first(item.product_name_snapshot, item.product_name, item.name, '—')}</strong>
                  <span className="a4-ltr">{first(item.product_sku_snapshot, item.sku_snapshot, item.product_sku, item.sku, '—')}</span>
                  {first(item.price_tier_name_snapshot, item.tier_name_snapshot, item.tier_name) && <small>{first(item.price_tier_name_snapshot, item.tier_name_snapshot, item.tier_name)}</small>}
                </div>
                <div className="invoice-detail-list__amount">
                  <span>{number(quantity)} × {money(unitPrice)}</span>
      ...
      <section className="invoice-detail-section">
        ...
        <div className="invoice-detail-list">
          {payments.length ? payments.map((payment, index) => (
            <article className="invoice-detail-list__item" key={first(payment.id, `${payment.payment_method}-${index}`)}>
              <div>
                <strong>{first(payment.method_name_snapshot, payment.method_name, payment.payment_method, payment.method, '—')}</strong>
                <span>{paymentStageLabel(first(payment.stage, payment.payment_stage))}</span>
                {first(payment.reference_number, payment.referenceNumber) && <small className="a4-ltr">{first(payment.reference_number, payment.referenceNumber)}</small>}
              </div>
              <div className="invoice-detail-list__amount">
                <strong>{money(first(payment.applied_amount, payment.amount, 0))}</strong>
                {first(payment.cash_received, payment.cashReceived) !== undefined && <span>المستلم {money(first(payment.cash_received, payment.cashReceived))}</span>}
                {first(payment.change_amount, payment.change) !== undefined && <span>الباقي {money(first(payment.change_amount, payment.change))}</span>}
              </div>
            </article>
      ...
      {preorder && (
        <section className="invoice-detail-section">
          <h3>الحجز المرتبط</h3>
          <div className="invoice-detail-grid">
            <DetailRow label="رقم الحجز" value={first(preorder.preorder_number, preorder.preorderNumber)} ltr />
            <DetailRow label="الحالة" value={statusLabel(preorder.status)} />
            <DetailRow label="العربون" value={money(first(preorder.deposit_paid, preorder.depositPaid, 0))} />
            <DetailRow label="مبلغ الاستلام" value={money(first(preorder.pickup_amount, preorder.pickupAmount, 0))} />
          </div>
        </section>
      )}
          ...
          <div className="invoice-detail-list">
            {returns.map((entry, index) => (
              <article className="invoice-detail-list__item" key={first(entry.id, index)}>
                <div>
                  <strong>{first(entry.return_number, entry.correction_number, `#${entry.id || index + 1}`)}</strong>
        ...
        <div className="invoice-detail-list">
          {receipts.length ? receipts.map((receipt, index) => (
            <article className="invoice-detail-list__item" key={first(receipt.id, index)}>
              <div>
                <strong className="a4-ltr">{first(receipt.receipt_number, receipt.receiptNumber, `#${receipt.id}`)}</strong>
                <span>{first(receipt.reference_type, receipt.type, 'إيصال')}</span>
              </div>
              <div className="invoice-detail-list__amount">
                <strong>{number(first(receipt.print_count, receipt.printCount, 0))} طلب طباعة</strong>
          ...
          <div className="invoice-detail-timeline">
            {printRequests.map((request, index) => (
              <div key={first(request.id, index)}>
                ...
                <small>{number(first(request.copies, 1))} نسخة{request.is_reprint || request.isReprint ? ' · إعادة طباعة' : ''}</small>
          ...
          <div className="invoice-detail-timeline">
            {auditTimeline.map((entry, index) => (
              <div key={first(entry.id, index)}>
                ...
                {first(entry.notes, entry.description) && <p>{first(entry.notes, entry.description)}</p>}
              </div>
            ))}
          </div>
        </section>
