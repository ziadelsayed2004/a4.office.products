...
function normalizePayment(payment) {
  return {
    ...
    method: first(payment.methodName, payment.method_name_snapshot, payment.method_name, payment.payment_method, payment.method, '—'),
...
function normalizeItem(item) {
  return {
    ...
    sku: first(item.productSku, item.product_sku_snapshot, item.sku_snapshot, item.product_sku, item.sku),
    tier: first(item.priceTierName, item.price_tier_name_snapshot, item.price_tier_name, item.tier_name_snapshot, item.tier_name),
    quantity: Number(first(item.quantity, item.qty, 0)),
    unitPrice: Number(first(item.unitPrice, item.unit_price_snapshot, item.unit_price, item.price, 0)),
...
export function normalizeThermalReceipt(receipt = {}, settings = {}) {
  ...
  const referenceType = String(first(
  ...
  const variant = referenceType.includes('PICKUP')
    ? 'pickup'
    : referenceType.includes('DEPOSIT') || (preorder.id && !invoice.id)
      ? 'deposit'
      : 'sale';

  const widthSetting = String(first(
  ...
  return {
    ...
    preorderNumber: first(source.preorder_number, preorder.preorder_number, preorder.preorderNumber),
    ...
    customerName: first(source.customerName, source.customer_name_snapshot, source.customer_name, invoice.customer_name, preorder.customer_name),
    customerPhone: first(source.customerPhone, source.customer_phone_snapshot, source.customer_phone, invoice.customer_phone, preorder.customer_phone),
    ...
    total: Number(first(source.total, source.total_amount, invoice.total, preorder.total_amount, 0)),
    depositPaid: first(source.depositPaid, source.deposit_paid, preorder.deposit_paid),
    remainingAmount: first(source.remainingAmount, source.remaining_amount, preorder.remaining_amount),
    ...
    showCustomer: String(first(printSettings.print_show_customer, true)) !== 'false',
    showPriceTier: String(first(printSettings.print_show_price_tier, true)) !== 'false',
    showQr: String(first(printSettings.print_show_qr, true)) !== 'false',
  };
}
...
export function ThermalReceipt({ receipt, settings, reprint = false, copyNumber = 1, copies = 1 }) {
  ...
  return (
    <article className={className} data-receipt-ready="true">
      <header className="thermal-receipt__brand">
        ...
        {copies > 1 && <small>النسخة {number(copyNumber)} من {number(copies)}</small>}
      </header>
      ...
      <section className="thermal-receipt__meta">
        ...
        {data.showCustomer && data.customerName && <ReceiptRow label="العميل" value={data.customerName} />}
        {data.showCustomer && data.customerPhone && <ReceiptRow label="الهاتف" value={data.customerPhone} ltr />}
      ...
      <section className="thermal-receipt__items">
        {data.items.map((item, index) => {
          ...
          return (
            <div className="thermal-receipt__item" key={item.id || `${item.productId || item.sku}-${index}`}>
              ...
              <ReceiptRow label={`${number(item.quantity)} × ${money(item.unitPrice)}`} value={money(lineTotal)} />
              {item.returnedQuantity > 0 && <small className="thermal-receipt__return">مرتجع: {number(item.returnedQuantity)}</small>}
            </div>
          );
        })}
      </section>
      ...
      <section className="thermal-receipt__totals">
        ...
        {data.depositPaid !== undefined && <ReceiptRow label="العربون المدفوع" value={money(data.depositPaid)} />}
        {data.remainingAmount !== undefined && <ReceiptRow label="المتبقي" value={money(data.remainingAmount)} />}
        <ReceiptRow className="thermal-receipt__grand-total" label="الإجمالي" value={money(data.total)} />
      </section>
      ...
      {data.payments.length > 0 && (
        <section className="thermal-receipt__payments">
          ...
          {data.payments.map((payment, index) => (
            <div className="thermal-receipt__payment" key={payment.id || `${payment.method}-${index}`}>
              <ReceiptRow
                label={`${payment.method}${payment.stage ? ` · ${stageLabels[payment.stage] || payment.stage}` : ''}`}
                value={money(payment.amount)}
              />
              {payment.cashReceived !== undefined && <ReceiptRow label="المستلم نقداً" value={money(payment.cashReceived)} />}
              {payment.change !== undefined && <ReceiptRow label="الباقي للعميل" value={money(payment.change)} />}
