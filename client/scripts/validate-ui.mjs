...
const pageFiles = requiredPages.map((page) => path.join(src, 'pages', `${page}.jsx`));
...
const combinedPages = pageFiles.map((file) => fs.readFileSync(file, 'utf8')).join('\n');
...
const authContext = read('src/app/AuthContext.jsx');
const printService = read('src/services/printService.js');
const browserPrintSettings = read('src/utils/browserPrintSettings.js');
const config = read('src/config/appConfig.js');
const translator = read('src/locales/t.js');
const receiptPreview = read('src/pages/Receipts.jsx');
const receiptPrint = read('src/pages/ReceiptPrint.jsx');
const thermalReceipt = read('src/components/ThermalReceipt.jsx');
const thermalReceiptCss = read('src/components/ThermalReceipt.css');
const pos = read('src/pages/POS.jsx');
const productLabelPrint = read('src/pages/ProductLabelPrint.jsx');
const productLabelCss = read('src/styles/ProductLabelPrint.css');
const printerSettings = read('src/pages/PrinterSettings.jsx');
const inventory = read('src/pages/Inventory.jsx');
const auditLogs = read('src/pages/AuditLogs.jsx');
const shiftSummary = read('src/pages/ShiftSummary.jsx');
...
assert(
  /useLayoutEffect\(\(\)\s*=>[\s\S]*document\.documentElement\.dataset\.theme\s*=\s*mode/.test(
    theme
  ),
  'The document theme attribute is applied before the browser paints.'
);
...
assert(!/StyledEngineProvider/.test(theme), 'ThemeConfig does not duplicate StyledEngineProvider.');
assert(
...
for (const stylesheet of expectedGlobalOrder) {
  ...
  assert(importIndex > previousImportIndex, `Global CSS import order includes ${stylesheet}.`);
...
assert(exists('src/config/appConfig.js'), 'A4 application configuration is centralized.');
assert(
...
for (const component of visualComponents) {
  assert(exists(`src/components/${component}.jsx`), `Shared component exists: ${component}.jsx.`);
  assert(
...
assert(!/\bsx\s*=/.test(combinedJsx), 'JSX presentation does not use sx props.');
assert(!/\bstyle\s*=/.test(combinedJsx), 'JSX presentation does not use inline style props.');
assert(
  !/document\.documentElement\.style\.colorScheme/.test(theme) &&
    /color-scheme:\s*light/.test(variables) &&
    /color-scheme:\s*dark/.test(variables),
  'Browser color scheme is declared by theme attributes and CSS.'
);
assert(
  !/\b(?:InputProps|InputLabelProps|inputProps|PaperProps)\s*=/.test(combinedSource),
  'Deprecated MUI props are absent.'
);
assert(!/:has\(/.test(combinedCss), 'CSS avoids fragile :has-based label logic.');
assert(
...
assert(/cloneElement\(children/.test(fieldComponent), 'Field injects native TextField props.');
assert(/ALWAYS_SHRINK_TYPES/.test(fieldComponent), 'Date and time fields force safe label shrink.');
assert(/slotProps:\s*\{[\s\S]*inputLabel/.test(fieldComponent), 'Field uses MUI 9 slotProps.');
assert(
...
assert(
  /MuiSelect-select\.MuiInputBase-input[\s\S]*justify-content:\s*flex-start/.test(rtl),
...
assert(/--control-height-mobile:\s*44px/.test(variables), 'Mobile field height token is 44px.');
...
assert(count(theme, /MuiDialog:/g) === 1, 'Theme has exactly one MuiDialog override.');
...
assert(/borderRadius:\s*['"]50%['"]/.test(theme), 'Icon buttons remain circular.');
...
assert(!/#1a73e8/i.test(combinedSource), 'Template blue is not imported into the A4 client.');
assert(
...
const usedVariables = new Set(
  [...combinedCss.matchAll(/var\(\s*(--[a-zA-Z0-9_-]+)/g)].map((match) => match[1])
);
...
assert(/--sidebar-width:\s*270px/.test(variables), 'Expanded sidebar width is 270px.');
assert(/--sidebar-collapsed-width:\s*72px/.test(variables), 'Collapsed sidebar width is 72px.');
assert(/--topbar-height:\s*64px/.test(variables), 'Topbar height is 64px.');
assert(/--content-max-width:\s*1440px/.test(variables), 'Content maximum width is 1440px.');
assert(
  /<AppBar\b/.test(mainLayout) && count(mainLayout, /<Drawer\b/g) === 2,
  'MainLayout uses AppBar plus mobile and desktop Drawers.'
);
assert(/<Breadcrumbs\s*\/>/.test(mainLayout), 'Breadcrumbs render once in MainLayout.');
assert(
...
assert(
  /mobile-record-list/.test(dataTable) && /@media \(max-width:\s*720px\)/.test(dataTableCss),
...
assert(
  /--confirm-dialog-max-width:\s*480px/.test(variables) && /confirm-dialog/.test(confirmDialog),
  'ConfirmDialog is capped at 480px.'
);

assert(/export\s+const\s+api/.test(service), 'Relocated service retains the named api export.');
assert(
...
assert(
  /a4_color_mode/.test(config) && /APP_CONFIG\.storageKeys\.colorMode/.test(theme),
  'Light/dark preference keeps the a4_color_mode key.'
);
assert(
  /a4_access_token/.test(config) && /APP_CONFIG\.storageKeys\.accessToken/.test(service),
  'API access token contract remains centralized and unchanged.'
);
assert(
  /dispatchEvent\(new Event\(AUTH_SESSION_CLEARED_EVENT\)\)/.test(service) &&
    /shouldClearSessionForStorageEvent/.test(authContext) &&
    /addEventListener\?\.\(['"]storage['"]/.test(authContext),
  'Session clearing reaches AuthContext for local and cross-window token removal.'
);
...
const requiredRoutes = [
  ...
  '/return-authorizations/:authorizationId/print',
];
for (const route of requiredRoutes) {
  const routePattern =
    route === '/'
      ? /<Route\s+path="\/"/
      : new RegExp(`(?:path="${route.slice(1)}"|path="${route.replaceAll('/', '\\/')}")`);
...
assert(
  JSON.stringify(flattenKeys(ar).sort()) === JSON.stringify(flattenKeys(en).sort()),
  'Arabic and future English dictionaries keep matching keys.'
);
assert(/from\s+['"]\.\/ar\.json['"]/.test(translator), 'Runtime translator reads Arabic only.');
assert(
  !/from\s+['"]\.\/en\.json['"]/.test(translator),
  'English dictionary is not loaded at runtime.'
);
assert(
  !/setLocale|toggleLanguage|languageSwitch|changeLanguage/i.test(combinedSource),
...
assert(
  /<ThermalReceipt\b/.test(receiptPreview) && !/QRCodeSVG|receipt-paper__meta/.test(receiptPreview),
  'Receipt preview reuses the canonical ThermalReceipt component.'
);
assert(
  /FAIL_CLOSED_BROWSER_PRINT_SETTINGS/.test(browserPrintSettings) &&
    count(browserPrintSettings, /auto_print_[a-z_]+:\s*['"]false['"]/g) === 3 &&
    count(browserPrintSettings, /print_show_[a-z_]+:\s*['"]false['"]/g) === 3 &&
    /FAIL_CLOSED_BROWSER_PRINT_SETTINGS/.test(pos) &&
    /\?\?\s*['"]false['"]/.test(pos),
  'Printer-setting failures disable automatic printing and sensitive receipt content.'
);
assert(
  /PRINTER_SETTINGS_UNAVAILABLE_MESSAGE/.test(receiptPrint) &&
    /PRINTER_SETTINGS_UNAVAILABLE_MESSAGE/.test(printService) &&
    !/printer-settings['"]\)\.catch\(\(\)\s*=>\s*\(\{\s*data:\s*\{\}/.test(receiptPrint),
  'Receipt printing blocks with an actionable error when safe settings are unavailable.'
);
assert(
  /function firstPresent/.test(thermalReceipt) &&
    /businessName:\s*firstPresent/.test(thermalReceipt) &&
    /footer:\s*firstPresent/.test(thermalReceipt) &&
    /receipt_printer_header\s*\?\?/.test(printerSettings),
  'Deliberately empty receipt headers and footers are preserved.'
);
assert(
  /\.thermal-receipt__row\s*>\s*strong\s*\{[\s\S]*max-width:\s*65%[\s\S]*overflow-wrap:\s*anywhere/.test(
    thermalReceiptCss
  ),
  'Long receipt values wrap within the thermal paper width.'
);
assert(
  /thermal-receipt--58/.test(read('src/components/ThermalReceipt.css')) &&
    /thermal-receipt--80/.test(read('src/components/ThermalReceipt.css')),
  'Thermal receipts preserve 58mm and 80mm output widths.'
);
assert(
  /applyProductLabelPageSize/.test(productLabelPrint) &&
    /38mm/.test(productLabelCss) &&
    /50mm/.test(productLabelCss) &&
    /80mm/.test(productLabelCss) &&
    /page-break-after:\s*auto/.test(productLabelCss),
  'Product labels use exact supported page sizes without a trailing blank page.'
);
assert(
  /print_mode:\s*['"]browser['"]/.test(printerSettings) &&
    !/Kiosk printing|receipt_printer_address|qr_printer_address/.test(printerSettings),
  'Printer settings expose Browser Print options without legacy device fields.'
);
assert(
  /limit:\s*String\(PAGE_SIZE\)/.test(inventory) &&
    /payload\.pagination\?\.total/.test(inventory) &&
    /limit:\s*String\(PAGE_SIZE\)/.test(auditLogs) &&
    /payload\.pagination\?\.total/.test(auditLogs),
  'Inventory and audit pages request and render server pagination metadata.'
);
assert(
  /Idempotency-Key/.test(inventory) && /Idempotency-Key/.test(shiftSummary),
  'Inventory adjustments and cash movements send idempotency keys.'
);
for (const page of ['AuditLogs', 'Inventory', 'Preorders', 'Products', 'Shifts']) {
  const pageSource = read(`src/pages/${page}.jsx`);
  assert(
    !/setTimeout\s*\(\s*load/.test(pageSource),
    `${page} filter reset does not reload through a stale state closure.`
  );
  assert(
    /const loadSequence = useRef\(0\)/.test(pageSource) &&
      /requestId !== loadSequence\.current/.test(pageSource),
    `${page} ignores out-of-order list responses.`
  );
}
assert(
  /envDir:\s*['"]\.\.['"]/.test(viteConfig),
  'Vite reads environment variables from the repository root.'
);
assert(
  /@media \(max-width:\s*980px\)/.test(read('src/styles/POSPage.css')) &&
    /@media \(max-width:\s*340px\)/.test(read('src/styles/POSPage.css')),
