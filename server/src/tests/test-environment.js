...
function safeSuiteName(value) {
  return String(value || 'suite').replace(/[^a-z0-9_-]+/gi, '-').toLowerCase();
}
...
export async function seedCatalogFixture(db) {
  ...
  if (!category || !priceTier) throw new Error('Migrations did not create the catalog seed prerequisites.');
  ...
  const product = await db.run(
    ...
    ['Test A4 notebook', 'NOTE-A4-TEST', '6220000000001', category.id, 'Isolated smoke-test fixture']
  );
  await db.run(
    'INSERT INTO product_prices (product_id, price_tier_id, price) VALUES (?, ?, ?);',
    [product.lastID, priceTier.id, 4000]
  );
