...
import {
  addCalendarDay, cairoMidnightUtc, getInvoiceDetail, listInvoices
...
function addDateRange(sql, params, column, filters) {
  if (filters.startDate) { sql.push(`${column} >= ?`); params.push(cairoMidnightUtc(filters.startDate)); }
  if (filters.endDate) { sql.push(`${column} < ?`); params.push(cairoMidnightUtc(addCalendarDay(filters.endDate))); }
}
...
export async function getAdminKPIs() {
  ...
  const [sales, deposits, preorders, lowStock, pendingShifts, topProducts, refunds] = await Promise.all([
    db.get('SELECT COALESCE(SUM(total), 0) total_sales, COUNT(*) sales_count FROM orders WHERE created_at >= ? AND created_at < ?;', [start, end]),
    db.get("SELECT COALESCE(SUM(applied_amount), 0) total_deposits FROM payments WHERE stage = 'PREORDER_DEPOSIT' AND direction = 'IN' AND is_excluded = 0 AND created_at >= ? AND created_at < ?;", [start, end]),
    db.get("SELECT COUNT(*) active_preorders_count FROM preorders WHERE status IN ('DEPOSIT_PAID_WAITING_STOCK', 'READY_FOR_PICKUP');"),
    ...
    db.get("SELECT COUNT(*) pending_shifts_count FROM shifts WHERE status = 'PENDING_ADMIN_REVIEW';"),
    db.all(`SELECT oi.product_id AS id, oi.product_name_snapshot AS name, oi.sku_snapshot AS sku,
                   SUM(oi.quantity) total_qty
              FROM order_items oi JOIN orders o ON o.id = oi.order_id
             WHERE o.created_at >= ? AND o.created_at < ?
             GROUP BY oi.product_id, oi.product_name_snapshot, oi.sku_snapshot
             ORDER BY total_qty DESC LIMIT 5;`, [start, end]),
    db.get('SELECT COALESCE(SUM(total_refunded), 0) total_refunds FROM returns WHERE created_at >= ? AND created_at < ?;', [start, end])
  ]);
  return {
    scopeDate: date,
...
export async function getInvoicesReport(filters = {}) {
  const listing = await listInvoices({ ...filters, limit: Math.min(Number(filters.limit) || 100, 100), offset: Number(filters.offset) || 0 });
  const rows = [];
  ...
  const totalRefunded = rows.reduce((sum, row) => sum + row.returns.reduce((returnSum, item) => returnSum + item.total_refunded, 0), 0);
  return {
    summary: {
...
export async function getPreordersReport(filters = {}) {
  ...
  if (filters.status) { clauses.push('pr.status = ?'); params.push(filters.status); }
  if (filters.cashierId) { clauses.push('pr.cashier_id = ?'); params.push(filters.cashierId); }
  if (filters.search) {
    const value = `%${String(filters.search).trim()}%`;
    clauses.push('(pr.customer_name_snapshot LIKE ? OR pr.customer_phone_snapshot LIKE ? OR pr.preorder_number LIKE ?)');
    params.push(value, value, value);
  }
  if (filters.availabilityPolicy) {
    clauses.push('EXISTS (SELECT 1 FROM preorder_items pi WHERE pi.preorder_id = pr.id AND pi.availability_policy_snapshot = ?)');
  ...
  const rows = await db.all(
    `SELECT pr.*, u.name cashier_name FROM preorders pr JOIN users u ON u.id = pr.cashier_id
      WHERE ${clauses.join(' AND ')} ORDER BY pr.created_at DESC LIMIT 500;`, params
  );
  for (const row of rows) {
    row.items = await db.all(
      `SELECT pi.*, pi.product_name_snapshot product_name, pi.sku_snapshot sku,
         ...
         FROM preorder_items pi JOIN products p ON p.id = pi.product_id WHERE pi.preorder_id = ?;`, [row.id]
    );
  }
  return {
    summary: {
...
export async function getInventoryReport(filters = {}) {
  ...
  const rows = await db.all(
    `SELECT p.*, c.name category_name,
      ...
      WHERE ${clauses.join(' AND ')} ORDER BY p.name;`, params
  );
  for (const row of rows) {
    row.open_preorder_quantity = Number(row.open_preorder_quantity || 0);
    row.can_sell_now = row.is_active === 1 && row.current_stock > 0 ? 1 : 0;
    row.can_preorder_now = row.is_active === 1
      && row.availability_policy === 'STOCK_WITH_PREORDER_WHEN_OUT_OF_STOCK'
      && row.current_stock === 0 ? 1 : 0;
  }
  const summary = {
    total_products: rows.length,
    low_stock_count: rows.filter((row) => row.current_stock > 0 && row.current_stock <= row.low_stock_threshold).length,
  ...
  let filtered = rows;
  if (filters.stockStatus === 'LOW_STOCK') filtered = rows.filter((row) => row.current_stock > 0 && row.current_stock <= row.low_stock_threshold);
  if (filters.stockStatus === 'OUT_OF_STOCK') filtered = rows.filter((row) => row.current_stock === 0);
...
export async function getPaymentsReport(filters = {}) {
  ...
  if (filters.stage) { clauses.push('p.stage = ?'); params.push(filters.stage); }
  if (filters.direction) { clauses.push('p.direction = ?'); params.push(filters.direction); }
  if (filters.method) { clauses.push('p.payment_method = ?'); params.push(filters.method); }
  if (filters.cashierId) { clauses.push('p.cashier_id = ?'); params.push(filters.cashierId); }
  if (filters.shiftId) { clauses.push('p.shift_id = ?'); params.push(filters.shiftId); }
  const rows = await db.all(
    `SELECT p.*, u.name cashier_name, s.status shift_status,
      ...
      WHERE ${clauses.join(' AND ')} ORDER BY p.created_at DESC LIMIT 1000;`, params
  );
  return {
    summary: {
      incoming: rows.filter((row) => row.direction === 'IN').reduce((sum, row) => sum + row.applied_amount, 0),
      outgoing: rows.filter((row) => row.direction === 'OUT').reduce((sum, row) => sum + row.applied_amount, 0),
...
export async function getShiftsReport(filters = {}) {
  ...
  if (filters.cashierId) { clauses.push('s.user_id = ?'); params.push(filters.cashierId); }
  if (filters.status) { clauses.push('s.status = ?'); params.push(filters.status); }
  const rows = await db.all(
    `SELECT s.*, u.name cashier_name, r.revision_number, r.system_totals_json,
      ...
      WHERE ${clauses.join(' AND ')} ORDER BY s.opened_at DESC LIMIT 500;`, params
  );
  for (const row of rows) {
...
export async function getCashiersReport(filters = {}) {
  ...
  const rows = await db.all(
    `SELECT u.id cashier_id, u.name cashier_name, COUNT(o.id) invoice_count,
      ...
      WHERE u.role IN ('Admin', 'Cashier') GROUP BY u.id, u.name ORDER BY invoice_total DESC;`, params
  );
  return { rows, summary: { cashier_count: rows.length, invoice_total: rows.reduce((sum, row) => sum + row.invoice_total, 0) } };
}
