...
async function movementTotals(connection, shiftId) {
  const rows = await connection.all(
    'SELECT type, COALESCE(SUM(amount), 0) AS total FROM cash_movements WHERE shift_id = ? GROUP BY type;',
    [shiftId]
  );
...
async function getShiftSystemSnapshot(connection, shift) {
  const methods = await connection.all('SELECT code FROM payment_methods ORDER BY sort_order, id;');
  ...
  totals.Cash = Number(shift.opening_cash) + (payments.Cash || 0) + (movements.PAY_IN || 0) - (movements.PAY_OUT || 0);
  return {
    methods: totals,
    movements: {
...
export async function openShift(userId, openingCash) {
  ...
  return withTransaction(async (connection) => {
    ...
    if (existing) throw new AppError('The current shift is pending Admin review.', 409, 'SHIFT_PENDING_REVIEW');
    let result;
    try {
      result = await connection.run(
        "INSERT INTO shifts (user_id, status, opening_cash) VALUES (?, 'OPEN', ?);", [userId, amount]
      );
    } catch (error) {
      if (error.code === 'SQLITE_CONSTRAINT') throw new AppError('An unfinished shift already exists.', 409, 'UNFINISHED_SHIFT_EXISTS');
      throw error;
    }
    await connection.run(
      `INSERT INTO cash_movements (shift_id, user_id, type, amount, notes)
       VALUES (?, ?, 'OPENING', ?, ?);`, [result.lastID, userId, amount, 'Opening cash']
    );
    await writeAuditLog({
      userId, shiftId: result.lastID, actionType: 'SHIFT_OPEN', entityType: 'shifts', entityId: result.lastID,
      afterValues: { openingCash: amount }, connection
    });
    return { shift: await connection.get('SELECT * FROM shifts WHERE id = ?;', [result.lastID]), resumed: false };
  });
}
...
export async function getCurrentShiftSummary(userId, connection = db) {
  ...
  const [snapshot, sales, movementsList, revisions] = await Promise.all([
    ...
    connection.get('SELECT COUNT(*) AS count, COALESCE(SUM(total), 0) AS total_amount FROM orders WHERE shift_id = ?;', [shift.id]),
    connection.all('SELECT * FROM cash_movements WHERE shift_id = ? ORDER BY id DESC;', [shift.id]),
    connection.all('SELECT * FROM shift_close_revisions WHERE shift_id = ? ORDER BY revision_number DESC;', [shift.id])
  ]);
  return {
    shift,
    sales,
    payments: Object.entries(snapshot.methods).map(([payment_method, total_amount]) => ({ payment_method, total_amount })),
    cashMovements: [
...
function normalizeActuals(actuals, activeMethods) {
  ...
  for (const method of activeMethods) {
    if (actuals[method.code] === undefined) {
      throw new AppError(`Actual total is required for ${method.code}.`, 400, 'SHIFT_METHOD_ACTUAL_REQUIRED');
    }
    result[method.code] = requirePiasters(actuals[method.code], `actuals.${method.code}`);
  }
  const invalid = Object.keys(actuals).filter((code) => !activeMethods.some((method) => method.code === code));
  if (invalid.length) throw new AppError(`Unknown payment methods: ${invalid.join(', ')}`, 400, 'UNKNOWN_SHIFT_PAYMENT_METHOD');
  return result;
}
...
export async function requestCloseShift(userId, closeData) {
  return withTransaction(async (connection) => {
    ...
    if (!shift) throw new AppError('No open shift is available to close.', 409, 'OPEN_SHIFT_REQUIRED');
    const activeMethods = await connection.all('SELECT code FROM payment_methods WHERE is_active = 1 ORDER BY sort_order, id;');
    ...
    const variances = Object.fromEntries(activeMethods.map(({ code }) => [code, actuals[code] - (system.methods[code] || 0)]));
    const row = await connection.get(
      'SELECT COALESCE(MAX(revision_number), 0) + 1 AS next FROM shift_close_revisions WHERE shift_id = ?;', [shift.id]
    );
    const revision = await connection.run(
      `INSERT INTO shift_close_revisions
       ...
       VALUES (?, ?, ?, 'SUBMITTED', ?, ?, ?, ?);`,
      [shift.id, row.next, userId, JSON.stringify(system), JSON.stringify(actuals), JSON.stringify(variances), closeData?.cashierNote || null]
    );
    const update = await connection.run(
      ...
      [
        system.methods.Cash || 0, system.methods.Card || 0, system.methods.InstaPay || 0,
        system.methods.Wallet || 0, system.methods.Transfer || 0, actuals.Cash || 0,
        actuals.Card || 0, actuals.InstaPay || 0, actuals.Wallet || 0,
        actuals.Transfer || 0, actuals.Cash || 0, shift.id
      ]
    );
    if (update.changes !== 1) throw new AppError('Shift state changed concurrently.', 409, 'SHIFT_STATE_CONFLICT');
    await writeAuditLog({
      userId, shiftId: shift.id, actionType: 'SHIFT_CLOSE_REQUEST', entityType: 'shift_close_revisions', entityId: revision.lastID,
      afterValues: { revisionNumber: row.next, actuals, variances }, connection
    });
    return {
      shift: await connection.get('SELECT * FROM shifts WHERE id = ?;', [shift.id]),
      revision: parseRevision(await connection.get('SELECT * FROM shift_close_revisions WHERE id = ?;', [revision.lastID]))
    };
  });
}
...
async function getSubmittedRevision(connection, shiftId) {
  ...
  if (shift.status !== 'PENDING_ADMIN_REVIEW') throw new AppError('Shift is not pending review.', 409, 'SHIFT_NOT_PENDING');
  const revision = await connection.get(
    `SELECT * FROM shift_close_revisions WHERE shift_id = ? AND status = 'SUBMITTED'
     ORDER BY revision_number DESC LIMIT 1;`, [shiftId]
  );
  if (!revision) throw new AppError('Submitted close revision not found.', 409, 'SHIFT_REVISION_NOT_FOUND');
  ...
  return withTransaction(async (connection) => {
    ...
    const reviewed = await connection.run(
      `UPDATE shift_close_revisions SET status = 'APPROVED', admin_id = ?, admin_reason = ?, reviewed_at = CURRENT_TIMESTAMP
       WHERE id = ? AND status = 'SUBMITTED';`, [adminId, adminNotes || null, revision.id]
    );
    if (reviewed.changes !== 1) throw new AppError('Revision was reviewed concurrently.', 409, 'SHIFT_REVIEW_CONFLICT');
    const update = await connection.run(
      `UPDATE shifts SET status = 'CLOSED', approved_close_revision_id = ?, admin_notes = ?, updated_at = CURRENT_TIMESTAMP
       WHERE id = ? AND status = 'PENDING_ADMIN_REVIEW';`, [revision.id, adminNotes || null, id]
    );
    if (update.changes !== 1) throw new AppError('Shift state changed concurrently.', 409, 'SHIFT_STATE_CONFLICT');
    const declared = JSON.parse(revision.declared_totals_json);
    await connection.run(
      `INSERT INTO cash_movements (shift_id, user_id, type, amount, notes)
       VALUES (?, ?, 'CLOSING', ?, ?);`, [id, shift.user_id, declared.Cash || 0, 'Approved closing total']
    );
    await writeAuditLog({
      userId: adminId, shiftId: id, actionType: 'SHIFT_APPROVE', entityType: 'shift_close_revisions', entityId: revision.id,
      afterValues: { adminNotes: adminNotes || null }, connection
    });
...
export async function rejectShiftClose(adminId, shiftId, reason) {
  ...
  if (!cleanReason) throw new AppError('A rejection reason is required.', 400, 'SHIFT_REJECTION_REASON_REQUIRED');
  return withTransaction(async (connection) => {
    ...
    const reviewed = await connection.run(
      `UPDATE shift_close_revisions SET status = 'REJECTED', admin_id = ?, admin_reason = ?, reviewed_at = CURRENT_TIMESTAMP
       WHERE id = ? AND status = 'SUBMITTED';`, [adminId, cleanReason, revision.id]
    );
    if (reviewed.changes !== 1) throw new AppError('Revision was reviewed concurrently.', 409, 'SHIFT_REVIEW_CONFLICT');
    const update = await connection.run(
      `UPDATE shifts SET status = 'OPEN', closed_at = NULL, admin_notes = ?, updated_at = CURRENT_TIMESTAMP
       WHERE id = ? AND status = 'PENDING_ADMIN_REVIEW';`, [cleanReason, id]
    );
    if (update.changes !== 1) throw new AppError('Shift state changed concurrently.', 409, 'SHIFT_STATE_CONFLICT');
    await writeAuditLog({
      userId: adminId, shiftId: id, actionType: 'SHIFT_REJECT', entityType: 'shift_close_revisions', entityId: revision.id,
      afterValues: { reason: cleanReason, resumedStatus: 'OPEN' }, connection
    });
    return getShiftDetails(id, connection);
  });
}

export async function registerCashMovement(userId, { type, amount, notes }) {
  if (!['PAY_IN', 'PAY_OUT'].includes(type)) throw new AppError('Movement type must be PAY_IN or PAY_OUT.', 400, 'INVALID_CASH_MOVEMENT');
  const value = requirePiasters(amount, 'amount', { allowZero: false });
  const cleanNotes = String(notes || '').trim();
  if (!cleanNotes) throw new AppError('Cash movement reason is required.', 400, 'CASH_MOVEMENT_REASON_REQUIRED');
  return withTransaction(async (connection) => {
    const shift = await connection.get(
      "SELECT * FROM shifts WHERE user_id = ? AND status = 'OPEN' ORDER BY id DESC LIMIT 1;", [userId]
    );
    ...
    await writeAuditLog({
      userId, shiftId: shift.id, actionType: 'CASH_MOVEMENT', entityType: 'cash_movements', entityId: result.lastID,
      afterValues: { type, amount: value, notes: cleanNotes }, connection
    });
    return { id: result.lastID, shiftId: shift.id, type, amount: value, notes: cleanNotes };
  });
}
...
export async function listAllShifts(filters = {}, connection = db) {
  ...
  if (filters.cashierId) { sql += ' AND s.user_id = ?'; params.push(filters.cashierId); }
  if (filters.startDate) { sql += ' AND s.opened_at >= ?'; params.push(`${filters.startDate} 00:00:00`); }
  if (filters.endDate) { sql += " AND s.opened_at < datetime(?, '+1 day')"; params.push(`${filters.endDate} 00:00:00`); }
...
export async function getShiftDetails(shiftId, connection = db) {
  const shift = await connection.get(
    `SELECT s.*, u.name AS cashier_name, u.username AS cashier_username
       FROM shifts s JOIN users u ON u.id = s.user_id WHERE s.id = ?;`, [shiftId]
  );
  if (!shift) throw new AppError('Shift not found.', 404, 'SHIFT_NOT_FOUND');
  ...
  return { shift, systemTotals: summary, closeRevisions: revisions.map(parseRevision), cashMovements: movements };
}
