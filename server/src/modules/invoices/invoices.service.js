...
const PRIMARY_RECEIPT_ORDER = `CASE WHEN r.reference_type = 'order_sale' THEN 0 ELSE 1 END, r.id DESC`;
...
export function cairoMidnightUtc(dateText) {
  ...
  if (validation.getUTCFullYear() !== year
...
function buildListWhere(filters = {}, { cashierId = null } = {}) {
  ...
  if (filters.receiptNumber) {
    clauses.push(`EXISTS (SELECT 1 FROM receipts r WHERE ${RECEIPT_MATCH} AND r.receipt_number LIKE ?)`);
  ...
  if (filters.paymentMethod) {
    clauses.push(`EXISTS (
      SELECT 1 FROM payments pay
       WHERE pay.is_excluded = 0
         AND COALESCE(pay.method_snapshot, pay.payment_method) = ?
         AND (
    ...
    params.push(String(filters.paymentMethod));
  }
  if (filters.origin) {
...
export async function resolveExactInvoiceId(criteria, connection = db) {
  const entries = ['token', 'invoiceNumber', 'receiptNumber']
    .filter((key) => typeof criteria?.[key] === 'string' && criteria[key].trim().length > 0);
  if (entries.length !== 1) {
    throw new AppError('Provide exactly one of token, invoiceNumber, or receiptNumber.', 400, 'EXACT_INVOICE_LOOKUP_REQUIRED');
  }
  ...
  if (entries[0] === 'token') {
    ...
    row = await connection.get('SELECT id FROM orders WHERE invoice_number = ?;', [criteria.invoiceNumber.trim()]);
  } else {
    row = await connection.get(
      `SELECT o.id
         FROM orders o
...
async function authorizeInvoice(invoice, actor, { credential = null, connection = db } = {}) {
  ...
  if (actor?.role !== 'Cashier') throw new AppError('Invoice access is forbidden.', 403, 'FORBIDDEN');
  ...
  throw new AppError('This invoice is outside the cashier\'s permitted scope.', 403, 'INVOICE_SCOPE_FORBIDDEN');
}
  ...
  const rows = await connection.all(
    `SELECT r.*, u.name AS printed_by_name, u.username AS printed_by_username
      ...
      ORDER BY CASE
        ...
        ELSE 1
      END, r.id DESC;`,
    [invoice.id, invoice.preorder_id || -1, invoice.id, invoice.origin, invoice.origin]
  );
  return rows.map((row) => ({ ...row, snapshot: parseJson(row.snapshot_json), snapshot_json: undefined }));
}
...
async function loadAuditTimeline(invoice, receipts, preorder, returns, connection) {
  const clauses = ["(al.entity_type IN ('order', 'orders', 'invoice', 'invoices') AND al.entity_id = ?)"];
  ...
  if (receipts.length > 0) {
    clauses.push(`(al.entity_type IN ('receipt', 'receipts') AND al.entity_id IN (${receipts.map(() => '?').join(', ')}))`);
    params.push(...receipts.map((receipt) => receipt.id));
  }
  if (returns.length > 0) {
    clauses.push(`(al.entity_type IN ('return', 'returns') AND al.entity_id IN (${returns.map(() => '?').join(', ')}))`);
...
export async function getInvoiceDetail(invoiceId, actor, { credential = null, connection = db } = {}) {
  ...
  const items = Array.isArray(primarySnapshot.items) && primarySnapshot.items.length > 0
    ? primarySnapshot.items.map(normalizeSnapshotItem)
    : savedItems;
  ...
  const printRequests = await connection.all(
    ...
    [invoice.id, linkedPreorderId || -1, invoice.id]
  );
  ...
  return {
    ...
    payments: payments.length > 0
      ? payments
      : (Array.isArray(primarySnapshot.payments) ? primarySnapshot.payments.map(normalizeSnapshotPayment) : []),
    preorder,
    returns,
    printRequests,
    auditTimeline
