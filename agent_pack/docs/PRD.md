...
| App language | Arabic-first UI; complete RTL release with locale-driven translation support |
...
4. [Language and compatibility rules](#language)
   - [4.1 Frontend experience contract](#frontend-experience)
...
## 4.1 Frontend experience contract

The existing repository template is the mandatory visual baseline. The final frontend keeps its compact Material dashboard character while applying the A4 blue/navy identity consistently.

Required frontend behavior:

- Desktop right sidebar and mobile right-side drawer.
- A4 blue/navy semantic theme with no purple template remnants.
- Complete light and dark modes with persisted user selection.
- Arabic is the default and complete release language.
- All visible strings come from locale files; English may be enabled with complete LTR parity.
- Shared page headers, KPI cards, filter panels, tables, drawers, dialogs, loading states, empty states, and error states.
- Dedicated responsive POS layout rather than simple generic stacking.
- Responsive validation at the viewports defined in `RESPONSIVE_DESIGN_MATRIX.md`.
- No product images.
- Thermal receipts and QR labels remain printer-safe and independent from screen theme.
...
- No MongoDB, Mongoose, or document-database persistence code in this project.
...
- Agent Pack executes one or two steps per run, controlled by `RUN_STEP_COUNT`, and never more than two.
...
The project must be implemented through `agent_pack/status.json` with one or two open/pending steps per run, controlled by `RUN_STEP_COUNT`.

Each run must:

1. Read `agent_pack/prompts/ONE_PROMPT_RUNNER.md`.
2. Select the next open/pending step; when `RUN_STEP_COUNT=2`, a second compatible step may run only after the first is verified and reported.
